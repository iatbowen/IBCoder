...
/*
 ...
 2）若符号来自动态库，打个标记，等启动的时候再说—交给dyld去加载和链接符号，把链接的过程推迟到运行时再进行，上面讲到的静态库符号冲突就可以推迟到运行时在解决
 在注册类时，运行时库会检查是否已经存在同名的类。如果已经存在同名的类，后加载的类会覆盖先加载的类。这是因为运行时库会更新符号表中的类指针，指向新加载的类。
