...
/*
 ...
 核心：只和直接对象通信，减少通信链路，强调对象间如何通信（通信视角）
 别名：最少知识原则
 实现：通过中介类解耦间接调用
 
 6、开闭封闭原则（Open Close Principle,简称OCP）
 核心：对扩展开放，对修改关闭
 手段：通过抽象和多态实现功能扩展
 目标：提升系统稳定性和可扩展性
 
 7、组合/聚合复用原则（Composite Reuse Principle，简称CRP）
 核心： 优先使用组合/聚合关系，而非继承关系来实现代码复用，强调类结构设计
 原因： 继承复用会暴露父类实现细节（白箱复用），破坏类的封装性，且父类变化必然影响子类，耦合度高
 区分：
 - 组合（Composition）：整体与部分同生命周期（Car — Engine）
 - 聚合（Aggregation）：整体与部分生命周期独立（School — Student）
 ...
 5）迪米特法则告诉我们只和直接对象通信，减少通信链路；
 6）开闭原则是总纲，他告诉我们要对扩展开放，对修改关闭；
 7）组合/聚合复用原则，告诉我们优先组合而非继承；
