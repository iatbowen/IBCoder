...
/*
 ...
 【同步 vs 异步】
 dispatch_sync  ：同步，阻塞当前线程，等任务完成再继续
 dispatch_async ：异步，不阻塞当前线程，任务扔进队列就继续往下走，具备开启新线程的能力
 ...
 时间片轮转：给每个线程分配一个固定的时间片(如 10ms),时间片用完就强制切换到下一个线程,循环往复。
 - 单个核心通过快速切换执行多个线程
 任务类型：
 ...
 为什么 CPU 要切换线程,而不是执行完一个再执行下一个?
 - 1. 响应性(最重要)：会一个线程"霸占" CPU，导致整个系统无响应
 - 2. 公平性：多个程序应该"公平"地获得 CPU 时间,而不是先到先得。
 - 3. 避免 IO 等待浪费 CPU
 - 4. 防止程序故障拖垮系统
