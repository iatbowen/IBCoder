...
/*
 ...
 读取文件数据（二进制 Data）
    ↓
 解码（Decode）                   把文件格式（JPEG、PNG、WebP）解析成像素数据（RGBA / BGRA）
    ↓
 解压缩（Decompress / Rasterize） 把压缩后的像素数据从压缩形式（如 DCT 块）转成实际内存中的像素位图
    ↓
 GPU 上传纹理（Texture Upload）    把 bitmap 上传到 GPU 显存中
    ↓
 屏幕显示（Render）
